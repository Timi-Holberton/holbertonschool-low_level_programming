...
{
	list_t *nœud_en_cours; /* Déclare un pointeur pour le nœud en cours */

	while (head != NULL) /* Tant que la liste n'est pas vide */
	{
		nœud_en_cours = head; /* Sauvegarde le nœud en cours */
		head = head->next; /* Avance au nœud suivant */
		free(nœud_en_cours->str); /* Libère la chaîne de caractères du nœud */
		free(nœud_en_cours); /* Libère la mémoire du nœud lui-même */
	}
}
